...
def AI_solutions(feedback):
    ...
    - Then you help them make a plan to execute their solutions
...
def AI_connection(solution, file, feedback, grade, name):
    ...
    - Choir: lalala@clubs.org. Leader - Veronica Lodge:
...
max_time = st.sidebar.slider("Passion level:", min_value=0, max_value=10)
max_time = st.sidebar.slider("Urgency level:", min_value=0, max_value=10)
...
if st.button("Brainstorm solutions"):
    with st.spinner("Brainstorming solutions..."):
        st.session_state.AI_sol_output = AI_solutions(feedback)
